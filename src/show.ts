...
/**
 * Detects pi's own session log format: every conversational line is wrapped
 * in `{ type: 'message', id, parentId, message: { role, ... } }`, unlike
 * Claude Code (flat `type: 'user' | 'assistant'`) or Codex (`payload`-keyed
 * rollout lines). Scans every line, not just the first, so it still works
 * when startLine/endLine slicing drops the leading `session` line.
 */
function isPiSession(lines: string[]): boolean {
  for (const line of lines) {
    if (line.includes('"omp_')) return false;
  }
  for (const line of lines) {
    let parsed: any;
    try {
    ...
    if (
      ...
      'parentId' in parsed &&
      ['user', 'assistant', 'toolResult', 'bashExecution'].includes(parsed.message.role)
    ) {
      return true;
    }
  }
...
function extractPiText(content: PiContentBlock[] | string | undefined): string {
...
function formatPiToolResultMarkdown(text: string, isError?: boolean): string {
  let output = isError ? '**Result (error):**\n' : '**Result:**\n';
  output += text.includes('\n') || text.length > 100 ? `\`\`\`\n${text}\n\`\`\`\n\n` : `${text}\n\n`;
  return output;
}

function formatPiConversationAsMarkdown(lines: string[]): string {
  const entries: PiEntry[] = lines.map(line => JSON.parse(line));

  const sessionEntry = entries.find(e => e.type === 'session');
  const messageEntries = entries.filter(
    (e): e is PiEntry & { message: PiMessagePayload } => e.type === 'message' && !!e.message
  );

  // Index tool results by call ID up front so tool calls can look up their
  // result directly instead of relying on message order.
...
function isCursorTranscript(lines: string[]): boolean {
  for (const line of lines) {
    try {
      const parsed = JSON.parse(line);
      if (parsed.type === 'status' || parsed.type === 'error') continue;
      if (parsed.type === undefined && parsed.role && parsed.message) return true;
      return false;
    } catch {
      continue;
    }
...
function formatCursorConversationAsMarkdown(lines: string[]): string {
  const entries = lines
