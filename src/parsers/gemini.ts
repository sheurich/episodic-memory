 ...
 */
function loadProjectMap(): Map<string, { name: string; dirPath: string }> {
  const map = new Map<string, { name: string; dirPath: string }>();
  const projectsFile = path.join(getGeminiDir(), 'projects.json');

  if (!fs.existsSync(projectsFile)) return map;

  try {
    const raw = JSON.parse(fs.readFileSync(projectsFile, 'utf-8'));
    const projects: Record<string, string> =
      typeof raw === 'object' && raw.projects ? raw.projects : raw;
...
export async function parseGeminiConversation(
  ...
  if (!session.messages || session.messages.length === 0) return exchanges;

  // Pair user→gemini exchanges
  let currentUser: { message: string; index: number; timestamp: string } | null = null;
  ...
  for (let i = 0; i < session.messages.length; i++) {
    ...
    if (msg.type === 'user') {
      ...
      while (lastGeminiIndex + 1 < session.messages.length &&
        ...
        if (nextText) assistantParts.push(nextText);
        if (next.toolCalls) {
...
export class GeminiSource implements ConversationSource {
