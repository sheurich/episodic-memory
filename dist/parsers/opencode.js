 ...
 */
function projectNameFromPath(dirPath) {
 ...
 * }
 */
export async function parseOpenCodeConversation(filePath, projectName, archivePath) {
    let data;
    try {
 ...
 */
function buildExchanges(session, messages, parts, projectName, archivePath) {
    const exchanges = [];
    ...
    for (let i = 0; i < userMessages.length; i++) {
        ...
        for (const aMsg of assistantMsgs) {
            ...
            if (aMsg.data.modelID)
                lastModel = aMsg.data.modelID;
            if (aMsg.data.providerID)
                lastProvider = aMsg.data.providerID;
...
export class OpenCodeSource {
    ...
    async discoverConversations() {
        ...
        try {
            ...
            const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('session','message','part','project')").all();
      ...
      `).all();
            ...
            const projects = tableNames.has('project')
                ? db.prepare('SELECT id, worktree, name FROM project').all()
                : [];
            ...
            for (const session of sessions) {
                ...
                const hasUser = db.prepare("SELECT 1 FROM message WHERE session_id = ? AND json_extract(data, '$.role') = 'user' LIMIT 1").get(session.id);
                if (!hasUser)
                    continue;
                // Export session data to a JSON file
                const messages = db.prepare('SELECT id, session_id, time_created, data FROM message WHERE session_id = ? ORDER BY time_created').all(session.id);
                ...
                if (messageIds.length > 0) {
                    ...
                    for (let i = 0; i < messageIds.length; i += batchSize) {
                        ...
                        const batchParts = db.prepare(`SELECT id, message_id, time_created, data FROM part WHERE message_id IN (${placeholders}) ORDER BY time_created`).all(...batch);
